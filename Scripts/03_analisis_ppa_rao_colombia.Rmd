...
pp_cold <- ppp(x = centroides_hot_coords[,1], y = centroides_hot_coords[,2], window = W)
...
rseq_bbox <- seq(0, rmax_bbox, length.out = 50) # 50 valores
...
pp_centroides <- ppp(x = coords[,1], y = coords[,2], window = W, marks = centroides_sf$lambda_area)

# Imágen de intensidad espacial usando kernel smoothing ponderado por la inversa del área
lambda_im <- density.ppp(pp_centroides, weights = marks(pp_centroides), sigma = bw.diggle(pp_centroides))
...
Kinhom_area_hot <- Kinhom(pp_hot, lambda = lambda_im, r = rseq_bbox)
Kinhom_area_cold <- Kinhom(pp_cold, lambda = lambda_im, r = rseq_bbox)
