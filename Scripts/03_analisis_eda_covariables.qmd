...
A continuación se calculan los índices geomorfométricos e hidroclimáticos. Primero se declaran los objetos raster y se visualizan para verificar. Todos los raster se visualizan en su resolución original pero se exportan a 50m de resolución.

### 2.1 DEM

El gradiente de elevación o la variabilidad de la elevación pueden contribuir al desarrollo de la pedodiversidad.
...
registro_lst_esp <- procesamiento_lotes_imagen(ucs_sf_4326, image = lst_img_mediana, start_idx = 29831, max_index = 30478, batch_s = 50, reduce_batch_by = 3, variable_name = "LST_media_espacial", scale = 50)

registro_lst_temp <- procesamiento_lotes_imagen(ucs_sf_4326, image = lst_cv_temporal, start_idx = 4206, max_index = 4355, batch_s = 20, reduce_batch_by = 3, variable_name = "LST_cv_temporal", scale = 50)
...
# Unir las coordenadas con los data frames de covariables
...
p_log_lst_mean_cv_dens

p_mosaico_distr_DEM <- p_dem_mean + p_log_dem_cv + p_log_dem_cv_dens + plot_layout(ncol = 3, widths = c(1, 1))
```

Se visualizan las distribuciones para LST con coeficiente de variación temporal

```{r}

p_lst_mean_cv <- ggplot(modelo_df, aes(lst_mean_temporal_cv)) +
  ggdist::stat_histinterval(fill = "#56B4E9", color = "black", .width = c(0.5, 0.9)) +
  geom_density(aes(y = after_stat(scaled)), color = "#FFD700", size = 0.8) +
  labs(title = "Distribución de CV media de LST)", y = "Frecuencia", x = "CV media de LST")

p_lst_mean_cv

p_log_lst_mean_cv <- ggplot(modelo_df, aes(log_lst_mean_temporal_cv)) +
  ggdist::stat_histinterval(fill = "#56B4E9", color = "black", .width = c(0.5, 0.9)) +
  geom_density(aes(y = after_stat(scaled)), color = "#FFD700", size = 0.8) +
  labs(title = "Distribución de log(CV media de LST)", y = "Frecuencia", x = "Log de CV media de LST")

p_log_lst_mean_cv

p_log_lst_mean_cv_dens <- ggplot(modelo_df, aes(log_lst_mean_temporal_cv_dens)) +
  ggdist::stat_histinterval(fill = "#56B4E9", color = "black", .width = c(0.5, 0.9)) +
  geom_density(aes(y = after_stat(scaled)), color = "#FFD700", size = 0.8) +
  labs(title = "Distribución de log(densidad de CV media de LST)", y = "Frecuencia", x = "Log de densidad CV media de LST")

p_log_lst_mean_cv_dens

p_mosaico_distr_LST_temp <- p_log_lst_mean_cv + p_log_lst_mean_cv_dens + plot_layout(ncol = 3, widths = c(1, 1))
```
...
p_mapa_lst_mean_cv_dens <- ggplot(modelo_df) +
  geom_sf(aes(fill = lst_mean_temporal_cv_dens), color = NA) +
  scale_fill_gradientn(colours = pal, na.value = "white") + 
  labs(title = "Mapa de log(densidad de medai de CV de LST)", fill = "Densidad CV") +
  theme_minimal()

p_mapa_lst_mean_cv_dens


p_mapa_log_lst_mean_cv_dens <- modelo_df |>
  mutate(lst_cv_dens_plus1 = lst_mean_temporal_cv_dens + 1) |>
  ggplot() +
  geom_sf(aes(fill = lst_cv_dens_plus1), color = NA) +
  scale_fill_gradientn(
    colours = pal,
    na.value = "white",
    trans = "log10",
  ...
  labs(title = "Mapa de log(densidad de media de CV de LST)", fill = "Densidad CV") +
  theme_minimal()

p_mapa_log_lst_mean_cv_dens
